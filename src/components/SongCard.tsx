...
export const SongCard: React.FC<SongCardProps> = ({ song, difficulty }) => {
    ...
    return (
        <div className={`
            bg-[#1a1a1a] text-white rounded-2xl p-8 max-w-sm mx-auto my-8 relative overflow-hidden border-4 animate-pop-in shadow-2xl w-full md:p-6 md:my-6
        ...
        `}>
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="bg-zinc-700 text-white px-2 py-1 rounded text-sm font-bold">
                    {difficulty.toUpperCase()}
                </div>
            </div>

            <h2 className="text-3xl font-bold leading-tight mb-2 text-white break-keep" title={song.title}>
                {song.title}
            </h2>

            {song.titleKo && song.title !== song.titleKo && (
                <div className="text-sm text-gray-400 mb-4">{song.titleKo}</div>
            )}

            <div className="flex flex-col gap-1 text-sm text-gray-300 mb-6">
                <span className="bg-zinc-800 border border-zinc-700 self-start px-2 py-1 rounded text-xs text-white">
                    {song.genre.join(', ')}
                </span>
            ...
            {course.images && course.images.length > 0 && (
                <div className="mt-4 border-t border-zinc-700 pt-4">
                    <button
                        className="w-full text-left text-lg font-bold text-gray-200 flex justify-between items-center py-2 hover:bg-white/5 rounded px-2 -mx-2 transition-colors"
